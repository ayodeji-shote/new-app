...
function PEdit() {
    ...
    function submitProperty() {
        let property = {
            id: id,
            type: checkproptype(inputtype.current.value),
    ...
    function checkproptype(proptype) {
        proptype = proptype.toLocaleLowerCase();
        proptype = proptype === (("semi") || ("apartment") || ("detached")) ? proptype.toUpperCase() : "";
        if (proptype === "") {
            alert("Invalid option please pick a valid one");
        }
        else {
            return (proptype);
        }
    ...
    return (
        <div className='pedit'>
            ...
            <input type="text" className='p1' id="propertytype" placeholder="Type" ref={inputtype} ></input>
